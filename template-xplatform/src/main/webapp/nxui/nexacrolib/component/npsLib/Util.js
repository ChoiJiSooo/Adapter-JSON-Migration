...
*******************************************************************************
 ...
 ● gfnGetType					: Component Type 반환
 ● gfnLength					: 입력값 형태에 따라서 길이 또는 범위를 구하는 함수
 ● gfnReplace					: 문자열을 대소문자 구별하여 치환 
 ● gfnChangeMessage				: Text를 치환하여 반환
 ● gfnToLower					: 문자열에 있는 모든 영어를 소문자로 변환
 ● gfnAddYear					: 입력된 날자에 offset 으로 지정된 만큼의 년(年)을 증감
 ● gfnToDate					: 입력된 날짜를 Date 객체로 변환
 ● gfnGetDateFormatString		: 날짜 포맷변환 함수
 ● gfnGetBirthYear				: 주민등록번호 뒷자리, 첫번째 번호로 년대를 반환
 ● gfnSubstr					: 원본 문자열에서 인수로 지정한 영역의 문자열을 잘라 반환  
 ● gfnDeleteRow					: 그리드 상태바 사용시 deleterow
*/
 ...
 * @example
 */ 
pForm.gfnLengthb = function(value)
{
	var sType;
	var nCount;

	if (this.gfnIsNull(value)) return 0;
 ...
 * @example
 */
pForm.gfnGetType = function(obj, isUpper)
{
 ...
 * @example
 */
pForm.gfnLength = function(value)
{
	if (this.gfnIsNull(value)) return 0;
	...
	if (value.components) {
		...
		if (this.gfnGetType(value) == "Dataset") return value.rowcount;
		return this.gfnNvl(value.length, 0); // Array, String, Variant인 경우
	}
};
 ...
 * @example
 */
pForm.gfnReplace = function(orgText, repFrom, repText)
{
	if (this.gfnIsNull(orgText)) return "";
	return String(orgText).split(repFrom).join(this.gfnNvl(repText, ""));
};
 ...
 * @example
 */
pForm.gfnChangeMessage = function(txt)
{
	if (this.gfnIsNull(txt)) return txt;
 ...
 * @example
 */
pForm.gfnAddYear = function(date, offset)
{
	if (this.gfnIsNull(date)) date = this.gfnGetDate();
	
	date = this.gfnToDate(date);
	date.addYear(offset);
	
	return this.gfnGetDateFormatString(date, "%Y%m%d");
};
 ...
 *
 */
pForm.gfnToDate = function(date)
{
	if (this.gfnIsNull(date)) return null;
 ...
 * @example
   this.gfnGetDateFormatString(new Date(), "%Y-%m-%d %H:%M:%S");
 */
pForm.gfnGetDateFormatString = function(objDate, format)
{
	if (this.gfnIsNull(format))	return "";
 ...
 * @example
 */
pForm.gfnGetBirthYear = function(resNo)
{
 ...
 * @example
 */
pForm.gfnSubstr = function(value, start, length)
{
	if (this.gfnIsNull(value)) return "";
...
// 그리드 상태바용 삭제
pForm.gfnDeleteRow = function (oDs, nRow) {

	if(oDs.getRowType(nRow) == 2) {
		oDs.deleteRow(nRow);
	} else {
		oDs.set_updatecontrol(false);
		oDs.setRowType(nRow, 8);
